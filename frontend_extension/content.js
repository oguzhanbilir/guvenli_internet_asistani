    ...
    if (skipAlwaysBtn) {
        skipAlwaysBtn.addEventListener("click", async () => {
            ...
            if (domain) {
                ...
                hideSecurityOverlay();
                // Sayfa içinde navigasyon olursa tekrar kontrol etmemek için
                // bir flag set edebiliriz ama şu an için sadece overlay'i kaldırmak yeterli
            } else {
...
// Sayfa yüklendiğinde ÖNCE whitelist kontrolü yap, sonra overlay göster
// ÖNEMLİ: Overlay'i göstermeden ÖNCE whitelist kontrolünün tamamen bitmesini bekliyoruz
(async () => {
    // Sayfa yüklenene kadar bekle (body element'i hazır olmalı)
    ...
    // Body hazır olana kadar bekle
    await waitForBody();
    
    // ÖNCE whitelist kontrolü yap - overlay göstermeden ÖNCE
    // Bu kontrol tamamen bitene kadar overlay gösterilmiyor
    const isWhitelisted = await checkWhitelistBeforeAnalysis();
    
    if (isWhitelisted) {
        // Domain whitelist'te, hiçbir şey yapma - overlay hiç gösterilmiyor
        console.log(`[Content] Domain whitelist'te: ${window.location.href}, overlay gösterilmiyor`);
        return;
    }
    
    // Whitelist'te değilse, overlay göster ve analiz yap
    // Artık whitelist kontrolü tamamen bitti, overlay güvenle gösterilebilir
    // Overlay sadece whitelist'te değilse oluşturulacak ve gösterilecek
    showSecurityOverlay();
    checkPageSecurity();
})();

function showSecurityOverlay() {
    // Body yoksa overlay gösterilemez
    if (!document.body) {
        console.warn("[Content] Body hazır değil, overlay gösterilemiyor");
        return;
    }
    ...
    // NOT: Bu fonksiyon sadece whitelist kontrolü TAMAMEN bittikten sonra çağrılıyor
    // Bu yüzden overlay güvenle gösterilebilir
...
async function checkPageSecurity() {
    ...
    // NOT: Whitelist kontrolü overlay gösterilmeden ÖNCE yapılıyor
    // Bu yüzden burada tekrar kontrol etmeye gerek yok
    // Eğer buraya geldiysek, domain whitelist'te değil demektir
