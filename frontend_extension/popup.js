...
async function analyzeUrl(url) {
    ...
    sendMessageWithTimeout({ action: "health" }, 5000)
    .then(healthCheck => {
        ...
        return sendMessageWithTimeout({
            ...
            url: url,
            includeVisual: true
        }, 30000); // Analiz için 30 saniye timeout
    })
    .then(result => {
    ...
    .catch(error => {
        ...
        if (!resultsShown && (!resultSec || resultSec.hidden)) {
            ...
            if (errorMessage.includes("Failed to fetch") || errorMessage.includes("NetworkError") || errorMessage.includes("ERR_CONNECTION_REFUSED") || errorMessage.includes("zaman aşımına uğradı") || errorMessage.includes("yanıt vermiyor")) {
...
async function removeFromWhitelist(domain) {
    try {
        ...
        console.log(`Domain whitelist'ten çıkarıldı: ${domain}`);
        return true;
    } catch (error) {
        console.error("Whitelist'ten çıkarma hatası:", error);
        return false;
    }
}
...
async function clearAllWhitelist() {
    try {
        await chrome.storage.local.set({ whitelisted_domains: [] });
        console.log("Tüm whitelist temizlendi");
        return true;
    } catch (error) {
        console.error("Whitelist temizleme hatası:", error);
        return false;
    }
}
...
async function renderWhitelist() {
    ...
    const clearAllBtn = document.getElementById("clear-all-whitelist-btn");
    ...
    if (whitelist.length === 0) {
        whitelistList.innerHTML = '<p class="whitelist-empty">Whitelist\'te henüz site yok.</p>';
    ...
    whitelistList.innerHTML = whitelist.map(domain => `
        <div class="whitelist-item">
            <span class="whitelist-item-domain">${domain}</span>
            <button class="remove-whitelist-btn" data-domain="${domain}">
                Çıkar
            </button>
        </div>
    `).join('');
    ...
    whitelistList.querySelectorAll('.remove-whitelist-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            if (confirm(`"${domain}" domain'ini whitelist'ten çıkarmak istediğinize emin misiniz?`)) {
                await removeFromWhitelist(domain);
                await renderWhitelist(); // Listeyi yenile
            }
        });
    });
}
...
function hideWhitelistSection() {
    const whitelistSection = document.getElementById("whitelist-section");
    if (whitelistSection) {
        whitelistSection.hidden = true;
    }
}
...
function init() {
    ...
    // Settings butonu (whitelist yönetimi)
    ...
    // Whitelist section kapatma butonu
    const closeWhitelistBtn = document.getElementById("close-whitelist-btn");
    if (closeWhitelistBtn) {
        closeWhitelistBtn.addEventListener("click", () => {
            hideWhitelistSection();
        });
    }
    ...
    if (clearAllBtn) {
        clearAllBtn.addEventListener("click", async () => {
            if (confirm("Tüm whitelist'i temizlemek istediğinize emin misiniz? Bu işlem geri alınamaz.")) {
                await clearAllWhitelist();
